...
import in.ghibliimage.DTO.TextGenerationRequestDTO;
import in.ghibliimage.Service.GhibliArtService;
...
public class GhibliGenerationController {
    ...
    public ResponseEntity<byte[]> generateGhibliArtFromText(
        ...
        try {
            byte[] imageBytes = ghibliArtService.createGhibliAartFromText ( requestDTO.getPrompt ( ) , requestDTO.getStyle ( ) );
            return ResponseEntity.ok ( ).contentType ( MediaType.IMAGE_PNG ).body ( imageBytes );
        } catch (Exception e) {
            e.printStackTrace ( );
            return ResponseEntity.internalServerError ( ).build ( );
